...
scaler.fit(df_train_X_train)
df_train_X_train = scaler.transform(df_train_X_train)
df_train_X_test = scaler.transform(df_train_X_test)
...
hist = model.fit(df_train_X_train, df_train_y_train, epochs=1000, batch_size=500, verbose=2, validation_split=0.4, callbacks=[es])
...
df_sub.to_csv(path + "submission_0116.csv", index=False)
