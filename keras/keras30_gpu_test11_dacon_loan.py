...
ip = Input(shape=(13, ))
d1 = Dense(19, activation='relu')(ip)
...
es = EarlyStopping(monitor='acc', mode='max', patience=50, verbose=20, restore_best_weights=True)

import time
start_time = time.time()


model.fit(X_train, y_train, epochs=10000, batch_size=15000, validation_split=0.15, callbacks=[es,mcp], verbose=2)
