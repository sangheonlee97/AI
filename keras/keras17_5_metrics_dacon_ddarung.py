...
tX_x_train, tX_x_test, tX_y_train, tX_y_test = train_test_split(tX_x, tX_y, test_size=0.15, random_state=8424808)
...
model_2.fit(tX_x_train, tX_y_train, epochs=1000, batch_size=5, validation_split=0.11, callbacks=[es])
...
X_train, X_test, y_train, y_test = train_test_split(X_t, y_t, test_size=0.11, random_state=243212343)
...
es = EarlyStopping(monitor='val_loss', mode='min', patience=200, verbose=1, restore_best_weights=True)
hist = model.fit(X_train, y_train, epochs=10000, batch_size=50, validation_split=0.11, callbacks=[es])
...
submission_csv.to_csv(path + "submission_0110_metrics.csv", index=False)
