...
for i in range(n):
    ...
    if i == 0:
        plt.text(-0.5, 0.5, f"Input", fontsize=10, ha='center', transform=ax.transAxes)

    ax = plt.subplot(3, n, i + 1 + n)
    plt.imshow(X_test_noised[idx].reshape(28, 28))
    plt.gray()
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    if i == 0:
        plt.text(-0.5, 0.5, f"Noise", fontsize=10, ha='center', transform=ax.transAxes)
    ...
    ax.get_yaxis().set_visible(False)
    if i == 0:
        plt.text(-0.5, 0.5, f"Output", fontsize=10, ha='center', transform=ax.transAxes)
