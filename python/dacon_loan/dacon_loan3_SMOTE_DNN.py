...
Scaler = RobustScaler(quantile_range=(0,75))
...
y_resampled = ohe.fit_transform(y_resampled)
...
model.fit(X_resampled, y_resampled, epochs=10000, batch_size=1500, validation_split=0.2, callbacks=[es])
######################## COMPILE, FIT End ########################
...
# print(sub_csv['대출등급'])
filename = "".join(["..//_data//_save//dacon_loan_3_SMOTE_DNN//dacon_loan_3_SMOTE_DNN", str(f1.round(4))])
model.save(filename + ".h5")
submission_csv.to_csv(path + "submisson_SMOTE_DNN.csv", index=False)
