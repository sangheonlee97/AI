...
def auto(rs, ts, bs, qr, test_csv):
    ...
    Scaler = RobustScaler(quantile_range=(26 - qr, 74 + qr))
    ...
    model.fit(X_train, y_train, epochs=10000, batch_size=bs, validation_split=0.2, callbacks=[es])
    ######################## COMPILE, FIT End ########################
